...
import '../user/CalendarView.css'; // Reusing the existing CSS
...
const ManagerCalendarView = () => {
  const { lists, fetchAllTasks } = useContext(ManagerTaskContext);
  ...
  const transformTasksToEvents = useCallback((tasks) => {
    ...
    console.log(`Found ${tasksWithDeadlines.length} tasks with deadlines out of ${tasks.length} total tasks`);
  ...
  // Fetch all tasks for manager - Using fetchAllTasks directly from context
  useEffect(() => {
    ...
    const cachedEvents = localStorage.getItem('managerCalendarEvents');
    if (cachedEvents) {
      try {
        ...
        fetchAllTasks().then(tasks => {
          ...
          localStorage.setItem('managerCalendarEvents', JSON.stringify(events));
        }).catch(error => {
    ...
    fetchAllTasks().then(tasks => {
      ...
      console.log("Fetched tasks for calendar:", tasks);
      if (!tasks || tasks.length === 0) {
        console.log("No tasks returned from fetchAllTasks");
      }
  ...
  }, [fetchAllTasks, userData?.username, isManagerLoggedIn, transformTasksToEvents]);
  ...
  if (isLoading) {
    return (
      <div className="calendar-loading">
        ...
        <p>Loading your manager calendar...</p>
      </div>
    );
  }

  // Memoized calendar options - FIXED DEPRECATED OPTIONS
  const calendarOptions = {
    ...
    lazyFetching: true,
    rerenderDelay: 10,
    // Replace deprecated eventLimit with updated option
    dayMaxEvents: true, // Replace eventLimit
    dayMaxEventRows: 3,  // Limit number of events per day
    moreLinkClick: "popover" // Replace eventLimitClick
  };
